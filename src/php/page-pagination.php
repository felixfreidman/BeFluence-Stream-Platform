<?php /* Template Name: Pagination Page */ ?>
<?php get_header(); ?>
<?php
    $pageList = $_GET['list'];
    $pageListInt = (int) ($pageList);

    $pageShow = 2;
    $pageSkipFormula = ($pageListInt - 1) * $pageShow;
    ...
    function get_lim_streamers($skip, $show) {
        global $wpdb;
        $sql = $wpdb->prepare("SELECT * FROM wp_specialData LIMIT %d, %d;", $skip, $show);
        return $wpdb->get_results($sql);
    }
    ...
    // Get limited number of streamers for LIMIT, OFFSET

    $streamers_array = get_lim_streamers($pageSkipFormula, $pageShow);

    // Get total number of streamers for calculations

    $streamers_array_total = get_all_streamers();

    $pageTotal = count($streamers_array_total);
    $pageCount = 0;

    // Getting total page numbers for limiting the pagination

    if($pageTotal % $pageShow !== 0) {
        $pageCount = intdiv($pageTotal,$pageShow) + 1;
    } else {
    ...
    $pageListNext = '/pagination?list=' . $pageList + 1;

    // Move back to main page if the pagination page is 2
...
<main class="main searchPage">
    <div class="main__row">
        <div class="main__pageHeader">Поиск</div>
    ...
    <div class="mainPage__tableNavigation">
        <div class="mainPage__tableNavigationButtons">
            <a href="<?php echo site_url() . $pageListPrev ?>" class="mainPage__tableNavigationButton">
                <svg width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg"
            ...
            <?php if($pageCount === $pageListInt) { ?>
                <a href="<?php echo site_url() . $pageListNext ?>" class="mainPage__tableNavigationButton disabled">
                    <svg width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg"
            ...
            <?php } else { ?>
                <a href="<?php echo site_url() . $pageListNext ?>" class="mainPage__tableNavigationButton">
                    <svg width="14" height="15" viewBox="0 0 14 15" fill="none" xmlns="http://www.w3.org/2000/svg"
                        class="mainPage__tableNavigationButtonArrow rightArrow">
                        <path d="M7.31445 2.4268L12.25 7.36231L7.31445 12.2978" stroke="white" stroke-linecap="round"
                            stroke-linejoin="round" />
