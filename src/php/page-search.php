...
<?php 
    $param_is_liked = $_GET['showLiked'];
    $param_platform = $_GET['platform'];
    $param_country = $_GET['country'];
    $param_is_gamble = $_GET['isGamble'];

    if($param_is_gamble === 'true') {
        $param_is_gamble = 'Yes';
    } else if($param_is_gamble === 'false') {
        $param_is_gamble = 'No';
    }

    $param_is_worked = $_GET['isWorked'];

    if($param_is_worked === 'true') {
        $param_is_worked = 'Yes';
    } else if($param_is_worked === 'false') {
        $param_is_worked = 'No';
    }

    $param_is_blocked = $_GET['isBlocked'];

    if($param_is_blocked === 'true') {
        $param_is_blocked = 'Yes';
    } else if($param_is_blocked === 'false') {
        $param_is_blocked = 'No';
    }

    $param_search_name = $_GET['searchName'];
    $search_query = 'SELECT * FROM `wp_specialData` ';
    $liked_query = '';
    $platform_query = '';
    $country_query = '';
    $gamble_query = '';
    ...
    function get_search_streamers() {
        global $wpdb;
        global $search_query;
        $total_query = set_queries();
        $search_query = $search_query . $total_query;
        $sql = $wpdb->prepare($search_query);
        return $wpdb->get_results($sql);
    }

    function set_queries() {
        global $param_is_liked;
        global $param_platform;
        global $param_country;
        global $param_is_gamble;
        global $param_is_worked;
        global $param_is_blocked;
        global $param_search_name;

        global $liked_query;
        global $platform_query;
        global $country_query;
        global $gamble_query;
        global $work_query;
        global $blocked_query;
        global $name_query;


        $total_query = '';

        if($param_is_liked !== 'false') {
            $liked_query = "`streamer_isLiked` = 'Yes'";
            $total_query = $total_query . $liked_query;
        }

        if($param_platform !== 'all') {
            if($liked_query !== '') {
                $platform_query = " AND `streamer_platform` = '". $param_platform ."'";
            } else {
                $platform_query = "`streamer_platform` = '". $param_platform ."'";
            }
            
            $total_query = $total_query . $platform_query;
        }

        if($param_country !== 'all') {
            if($platform_query !== '' || 
            $liked_query !== '') {
                $country_query = " AND `streamer_country` = '". $param_country ."'";
            } else {
                $country_query = "`streamer_country` = '". $param_country ."'";
            }
            
            $total_query = $total_query . $country_query;
        }

        if($param_is_gamble !== 'all') {
            if($platform_query !== '' || 
            $liked_query !== '' ||
            $country_query !== '') {
                $gamble_query = " AND `streamer_isGamble` = '". $param_is_gamble ."'";
            } else {
                $gamble_query = "`streamer_isGamble` = '". $param_is_gamble ."'";
            }
            
            $total_query = $total_query . $gamble_query;
        }

        if($param_is_worked !== 'all') {
            if(
            $platform_query !== '' || 
            $liked_query !== '' ||
            $country_query !== '' ||
            $gamble_query !== ''
                ...
                $work_query = " AND `streamer_isWorked` = '". $param_is_worked ."'";
            } else {
                $work_query = "`streamer_isWorked` = '". $param_is_worked ."'";
            }
            
            $total_query = $total_query . $work_query;
        }

        if($param_is_blocked !== 'all') {
            if(
            $platform_query !== '' || 
            $liked_query !== '' ||
            $country_query !== '' ||
            ...
            $work_query !== ''
                ...
                $blocked_query = " AND `streamer_isBlocked` = '". $param_is_blocked ."'";
            } else {
                $blocked_query = "`streamer_isBlocked` = '". $param_is_blocked ."'";
            }
            
            $total_query = $total_query . $blocked_query;
        }

        if($param_search_name !== 'false') {
            if(
            $platform_query !== '' || 
            $liked_query !== '' ||
            $country_query !== '' ||
            ...
            $blocked_query !== ''
                ...
                $name_query = " AND `streamer_name` LIKE '%". $param_search_name ."%'";
            } else {
                $name_query = "`streamer_name` LIKE '%%". $param_search_name ."%%'";
            }
            
            $total_query = $total_query . $name_query;
        }

        if($total_query !== '') {
            $total_query = 'WHERE ' . $liked_query . $platform_query . $country_query . $gamble_query . $work_query . $blocked_query . $name_query;
        }
        return $total_query;
    }
    ...
    function get_lim_streamers() {
        global $wpdb;
        $sql = $wpdb->prepare("SELECT * FROM wp_specialData LIMIT 2");
        return $wpdb->get_results($sql);
    }
    ...
    $streamers_array = get_search_streamers();
    $streamers_array_total = get_all_streamers();
?>

<div class="jsInfo jsHidden">
    <span id="querySearchIsLiked"><?php echo $param_is_liked ?></span>
    <span id="querySearchPlatform"><?php echo $param_platform ?></span>
    <span id="querySearchCountry"><?php echo $param_country ?></span>
    <span id="querySearchIsGamble"><?php echo $param_is_gamble ?></span>
    <span id="querySearchisWorked"><?php echo $param_is_worked ?></span>
    <span id="querySearchIsBlocked"><?php echo $param_is_blocked ?></span>
    <span id="querySearchName"><?php echo $param_search_name ?></span>
</div>
<main class="main mainPage searchPage">
    <div class="main__row">
        <div class="main__pageHeader">
            Поиск
            <?php if($param_is_liked !== 'false') {?>
                cреди отмеченных блогеров
            <?php } ?>
        </div>
    ...
    <?php if(count($streamers_array) !== 0) {?>
        ...
        <div class="mainPage__bloggersTable">
            <div class="bloggersTable__mainRow">
                ...
                <div class="bloggersTable__counter">
                    ...
                    &nbsp;из&nbsp;
                    <span><?php echo count($streamers_array_total) ?></span>
                </div>
            </div>
    ...
    <div class="mainPage__layout mainPage__layoutActive mainPage__layoutGrid">
        ...
        <?php foreach ( $streamers_array as $result ) { ?>
            <a class="mainPage__bloggersGridTile gridTile" href="<?php echo site_url() . '/streamer?streamer=' . $result->ID ?>">
    ...
    <?php } else { ?>
        <div class="mainPage__layout mainPage__layoutTable jsHidden" id="jsPrority">
            <div class="mainPage__bloggersTable">
                <div class="bloggersTable__mainRow">
                    <div class="bloggersTable__mainHeader">Блогеры</div>
                    <div class="bloggersTable__counter">
        ...
        <div class="mainPage__layout mainPage__layoutGrid jsHidden" id="jsPrority">
            <div class="mainPage__bloggersGrid">
            <?php foreach ( $streamers_array as $result ) { ?>
                <a class="mainPage__bloggersGridTile gridTile" href="<?php echo site_url() . '/streamer?streamer=' . $result->ID ?>">
                    <div class="gridTile__mainInfoContainer">
        ...
        <span class="errorHeader">Nothing found</span>
    <?php } ?>
    ...
    <div class="mainPage__filterContainer filterContainer jsHidden">
        ...
        <div class="filterContainer__filters">
            ...
            <div class="filterContainer__row">
                <label for="set_platform" class="filterContainer__selectLabel">
                    ...
                    <select name="set_platform" id="set_platform" class="filterContainer__select">
                        <?php if($param_platform === 'all') { ?>
                            <option value="all" selected>Все</option>
                        <?php } else { ?>
                            <option value="all">Все</option>
                        <?php } ?>
                        <?php foreach ( $platformsArray as $result ) { ?>
                            <?php if($param_platform === $result) { ?>
                ...
                <label for="set_country" class="filterContainer__selectLabel">
                    ...
                    <select name="set_country" id="set_country" class="filterContainer__select">
                        <?php if($param_country === 'all') { ?>
                            <option value="all" selected>Все</option>
                        <?php } else { ?>
                            <option value="all">Все</option>
                        <?php } ?>
                        <?php foreach ( $countriesArray as $result ) { ?>
                            <?php if($param_country === $result) { ?>
                                <option value="<?php echo $result?>" selected><?php echo $result?></option>
                            <?php } else { ?>
                                <option value="<?php echo $result?>"><?php echo $result?></option>
                            <?php } ?>
                ...
                <label for="set_gamble" class="filterContainer__selectLabel">
                    ...
                    <select name="set_isGamble" id="set_isGamble" class="filterContainer__select">
                        <?php if($param_is_gamble === 'all') { ?>
                            <option value="all" selected>Все</option>
                        <?php } else { ?>
                            <option value="all">Все</option>
                        <?php } ?>
                        <?php if($param_is_gamble === 'Yes') { ?>
                            <option value="true" selected>Готов</option>
                        <?php } else { ?>
                            <option value="true">Готов</option>
                        <?php } ?>
                        <?php if($param_is_gamble === 'No') { ?>
                            <option value="false" selected>Не готов</option>
                        <?php } else { ?>
                ...
                <label for="set_isWorked" class="filterContainer__selectLabel">
                    ...
                    <select name="set_isWorked" id="set_isWorked" class="filterContainer__select">
                        <?php if($param_is_worked === 'all') { ?>
                            <option value="all" selected>Все</option>
                        <?php } else { ?>
                            <option value="all">Все</option>
                        <?php } ?>
                        <?php if($param_is_worked === 'Yes') { ?>
                            <option value="true" selected>Работали</option>
                        <?php } else { ?>
                            <option value="true">Работали</option>
                        <?php } ?>
                        <?php if($param_is_worked === 'No') { ?>
                            <option value="false" selected>Не работали</option>
                        <?php } else { ?>
                            <option value="false">Не работали</option>
                        <?php } ?>
                    </select>
                </label>
                <label for="set_isBlocked" class="filterContainer__selectLabel">
                    ...
                    <select name="set_isBlocked" id="set_isBlocked" class="filterContainer__select">
                        <?php if($param_is_blocked === 'all') { ?>
                            <option value="all" selected>Все</option>
                        <?php } else { ?>
                            <option value="all">Все</option>
                        <?php } ?>
                        <?php if($param_is_blocked === 'Yes') { ?>
                            <option value="true" selected>В блоке</option>
                        <?php } else { ?>
                            <option value="true">В блоке</option>
                        <?php } ?>
                        <?php if($param_is_blocked === 'No') { ?>
                            <option value="false" selected>Не в блоке</option>
                        <?php } else { ?>
                            <option value="false">Не в блоке</option>
                        <?php } ?>
                    </select>
                </label>
            </div>
            <div class="filterContainer__row">
                ...
                <div class="filterContainer__rowButton filterContainer__rowButtonSP">Сохранить</div>
            </div>
        </div>
    </div>
</main>
